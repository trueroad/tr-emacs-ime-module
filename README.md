...
* GNU 公式のプレテスト配布バイナリ 28.0.5 でも動作するが、おかしい
    * 例えば C-\\ で IME/IM ON/OFF 切り替えても
      モードラインの表示が追従しなかったり反対になったりするなどします
        * Alt- 半角/全角だとおかしくならないように見えます
    * Emacs 28 だと現状のモジュールに実装した機能だけであれば
      既に本体に取り込まれているため、
      ビルドせずに Lisp だけ入れれば一応動くようにしてあります
        * 将来的にモジュールでの拡張を要する機能を実装する可能性はあります
